...
Check that the configured location is inside the selected forecast provider coverage. MET Norway and SMHI do not cover every coordinate, and Rain Radar marks unsupported locations as outside coverage when the provider reports that state.
...
Rain Radar sends a descriptive User-Agent and uses cache headers and conditional requests where supported. If a provider returns rate limits or temporary errors, wait for the next scheduled update.
