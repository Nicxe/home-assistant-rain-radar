"""Frontend resource management for the Rain Radar card."""
...
def _local_www_card_path(hass: HomeAssistant) -> Path:
    """Return target path for /local card serving."""
...
def _url_base(url: str) -> str:
    """Return URL without query or fragment."""
    split = urlsplit(url)
    return urlunsplit((split.scheme, split.netloc, split.path, "", ""))


def _url_with_version(base_url: str, cache_key: str) -> str:
    """Set the cache-busting query parameter."""
...
async def _async_ensure_card_resource(hass: HomeAssistant) -> bool:
    """Create or update Lovelace module resource for the card."""
    ...
    if resources is None:
        _LOGGER.debug("Lovelace resources API unavailable for %s", desired_url)
        return False
...
def _async_component_loaded_listener(hass: HomeAssistant) -> Callable[[], None]:
    """Create listener for late Lovelace/frontend component startup."""
...
async def async_setup_frontend(hass: HomeAssistant) -> None:
    """Sync the card file and ensure Lovelace resource registration."""
